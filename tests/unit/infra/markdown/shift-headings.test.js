...
describe('#shiftHeadings', () => {
  test('promotes a section heading to h1 (heading-xl)', () => {
    ...
    expect(render('#### 1.1.1 Deep', 3)).toContain('class="govuk-heading-xl"')
  })

  test('preserves relative nesting when shifting', () => {
    const html = render('## 1 Top\n\n### 1.1 Child', 1)
    expect(html).toContain('class="govuk-heading-xl"')
    expect(html).toContain('>1 Top</h1>')
    expect(html).toContain('class="govuk-heading-l"')
    expect(html).toContain('>1.1 Child</h2>')
  })
  ...
  test('leaves headings untouched when by is 0', () => {
    expect(render('### Heading', 0)).toContain('class="govuk-heading-m"')
  })
})
